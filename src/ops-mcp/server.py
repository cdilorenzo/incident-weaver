...
def get_service_health(service: str) -> ServiceHealth:
    return invoke_connector(lambda: connector.get_service_health(service))
...
def get_logs(service: str, time_range: TimeRange) -> LogResult:
    return invoke_connector(lambda: connector.get_logs(service, time_range))
...
def get_deployment(service: str) -> Deployment:
    return invoke_connector(lambda: connector.get_deployment(service))
...
def get_known_incidents(service: str) -> KnownIncidentResult:
    return invoke_connector(lambda: connector.get_known_incidents(service))
